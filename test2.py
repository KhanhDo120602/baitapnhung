...
def preprocess_input(frame):
    ...
    img = np.expand_dims(img, axis=0)  # Ensure the input has shape (1, height, width, channels)
    img = np.expand_dims(img, axis=-1)  # Ensure the input has shape (1, height, width, 1) for grayscale
    img = img / 255.0
    return img
